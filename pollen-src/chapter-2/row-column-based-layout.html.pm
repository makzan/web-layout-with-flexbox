...
In grid system, mobile first means we use the ◊code{small-*} layout modifiers by default. When we need to build a wider layout, we start using the ◊code{medium-*}, ◊code{large-*} and even ◊code{xlarge-*}. Each one overrides the smaller one.
